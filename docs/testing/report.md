...
The baseline was frozen in local commit `d24a34d` before properties or mutation regressions. Its [identity record](evidence/baseline-20260925T102731Z-62389.json), [JUnit](evidence/baseline-20260925T102731Z-62389-junit.xml), [coverage JSON](evidence/baseline-20260925T102731Z-62389-coverage.json), [coverage XML](evidence/baseline-20260925T102731Z-62389-coverage.xml), and [coverage HTML](evidence/baseline-20260925T102731Z-62389-coverage-html/index.html) preserve the selection and hashes. **24 cases passed**. Across schedule (4/4), Garmin extraction (22/32) and targets (17/24), 43/60 branches were covered (71.7%). This is scoped service evidence, not app-wide coverage.

## 3. Generated properties

The expanded suite leaves the frozen baseline files unchanged and adds 13 property functions: four schedule, six Garmin, three PostgreSQL-backed nutrition. Repeatable settings use `derandomize=True` with 100 examples per pure property (1,000 configured across ten functions) and 20+15+15 for database properties (50 configured). These are **1,050 configured generated examples**, distinct from 37 pytest cases. No generated failure occurred, so no fabricated shrunk counterexample or production calculation fix is claimed. Database properties use `deadline=None` because database round-trip time varies; no health check was suppressed.

The properties cover valid schedule order, normalization/rejection, Garmin trusted values and unrelated-key invariance, precedence, finite macro-consistent targets, nondecreasing protein/carbohydrate with weight, maintenance-window invariance, and date-isolated overrides. Garmin strategies generate bounded dictionaries with omitted and null fields, numeric strings, body-battery point lists, readiness lists, and max-metrics lists; trusted extracted values must remain finite and within the documented range. NaN/infinity are separate in-process robustness inputs, not JSON. Every generated database example calls guarded `reset_and_seed()` inside its body; the history-order property resets a second time before reinserting rows in reverse order. A function-scoped pytest fixture alone would not isolate examples. The [expanded evidence](evidence/expanded-20260925T102831Z-62901.json) records **37 passed**, 20.80 seconds of pytest time, and 53/60 scoped branches (88.3%) on the same measured service source hash as baseline. [Expanded coverage XML](evidence/expanded-20260925T102831Z-62901-coverage.xml) and [HTML](evidence/expanded-20260925T102831Z-62901-coverage-html/index.html) are retained.

## 4. Mutation comparison

Mutmut 3.8.0 is pinned in `uv.lock`. Generated workspaces copy the full `app` support package and selected tests, and import `mutants/app`. The clean test run and forced-fail instrumentation check passed; a [selected smoke mutant](mutation-smoke.md) was killed. Three fresh workspaces use two workers, a 300-second limit, the same schedule/Garmin source hash `8fb68ea0a8a4bc021d9489ad76dbcf795622fffee9511a97df5a8467ba31ff34`, and exactly the same 466 mutant IDs/diffs. Stage A recorded revision `d24a34d`, while B/C recorded `8c5148a`; those later commits changed tests and frontend code but not either mutated source file. [A](evidence/mutation-A-20260925T100433Z-52813/), [B](evidence/mutation-B-20260925T103100Z-64279/) and [C](evidence/mutation-C-20260925T103240Z-64278/) retain configurations, all-ID outcomes, generated mutant source and durations.
...
| B: A + schedule/Garmin properties | 265 | 201 | 0 | 0 | 265/466 = 56.9% |
| C: B + GAR-01/03 regressions | 287 | 179 | 0 | 0 | 287/466 = 61.6% |

The resolved score is killed ÷ (killed + survived + no-tests) across 466 distinct executable IDs. No equivalent adjustment is claimed. B kills 17 additional mutations in calorie, body-battery, readiness, and VO₂-max extraction after generated list and omission shapes were added. C kills 22 more: a nested sleep path, body-battery precedence, and raw-payload retention are meaningful observed behavior missing from weaker tests. `x_extract_sleep__mutmut_11`, for example, removes the preferred `dailySleepDTO.sleepTimeSeconds` path; the new 7,200/3,600-second conflict test catches the changed 2 h to 1 h result. `x__build__mutmut_2` drops a missing metric's payload; the regression checks that diagnostic payload. `x_extract_training_status__mutmut_21` changes only detail text for empty payloads while leaving status and scalar meaning intact, so no brittle exact-message test was added just for score. Other survivors remain individually unresolved; none is called equivalent without proof. See [mutation analysis](mutation-analysis.md) for representative diffs/dispositions.

## 5. Live API and browser workflows

The [isolated live-stack run](evidence/20260925T102915Z-63179.json) passed **five API/database checks and four Chromium checks**, with zero retries or flaky passes. [API JUnit](evidence/20260925T102915Z-63179-backend-smoke.xml) and [Playwright JSON](evidence/20260925T102915Z-63179-playwright.json) are distinct from mutation evidence. Each API case resets synthetic committed rows; each browser case resets rows and gets a fresh context. Browser noon UTC and backend athlete date are fixed. Polling uses bounded conditions; a real server, rather than a synchronous test client, observes background-job completion.
